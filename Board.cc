...
string Board::printTiles() {
    ...
    for (int i = 0; i < 2*TeamTileCount; ++i) { //go through the board
...
bool Board::moveSelectedTile(int dirX, int dirY) {
    ...
    if(spotTakenByOther){
        ...
        if(spotTakenByOtherEx) {
            ...
            tileSelectedObject->moveTile(dirX, dirY, dirBias); //move the selected tile according to its corrected x and y position
        }
    }
...
Tile * Board::bestTile(int playerID) {
    tuple<string, int> maxMove;
    Tile *wantedTile;
    maxMove = make_tuple("cats are nice", -100);
    for (Tile* t: tiles) { //for tile in tiles
        if(t->valid) { //if the tile is a valid tile (on the board)
            if (t->player == playerID) { //if it is the correct player
                vector<tuple<string, int> > moves = possibleMoves(t); //create a vector of its possible moves
                if (moves.size()) {
                    for (int i = 0; i < moves.size(); i++) {
                        if (get<1>(maxMove) < get<1>(moves[i])) { //if the tile has a better possible move, update the tile we want
                            maxMove = moves[i];
                            wantedTile = t;
                        }
                    }
    ...
    cout << wantedTile->name << endl;
    return wantedTile;
}

string Board::bestMove(Tile *tile) {
    tuple<string, int> maxMove; // tuple for the best move, includes move, weight of move
    string returnX, returnY, final;
    maxMove = make_tuple("INIT", -100); //init the tuple to be later overwritten
    vector< tuple <string, int> > moves = possibleMoves(tile);
    unsigned long size = moves.size();
    if(size) {
        for (int i = 0; i < moves.size(); i++) {
            if (get<1>(maxMove) < get<1>(moves[i])) { //if there is a better move then update the move me want
                maxMove = moves[i];
            }
        }
    }
    ...
    smatch payload;
    regex expression("([01])-([0-9]+)-([0-9]+)"); // regex for the move string
    regex_search(get<0>(maxMove), payload, expression);
    // set all of the tile related info
    // playerid will automatically be checked with regex to be 0 or 1
    int x = atoi(payload[2].str().c_str()); //convert the x into a int
    int y = atoi(payload[3].str().c_str()); //convert the y into a int
    x = tile->x - x; //subtract the new x from the original position
    y = tile->y - y; //subtract the new y from the original position
    final = to_string(tile->player) + "-'" + to_string(x) + "'-'" + to_string(y) + "'";
    cout << final << endl;
    return final;
}

vector<tuple <string, int> > Board::possibleMoves(Tile *currTile) {
    vector<tuple <string, int> > moves; // vector of moves with weights that determine how good of a move they actually are
